...
function broadcastMesasAtualizadas() {
  db.query('SELECT * FROM mesas_infantil ORDER BY id').then(result => {
    const payload = JSON.stringify({
      type: 'mesas_infantil:update',
      mesas_infantil: result.rows
    });
...
app.get('/api/mesas', async (req, res) => {
  const result = await db.query('SELECT * FROM mesas_infantil ORDER BY id');
...
app.post('/api/mesas/:id/reservar', async (req, res) => {
  try {
    ...
    const result = await db.query(
      'UPDATE mesas_infantil SET nome = $1, cadeiras = $2, ocupada = true WHERE id = $3 AND ocupada = false RETURNING *',
      [nome, cadeiras, id]
    );
    ...
    res.status(200).json({ message: 'Mesa reservada com sucesso.', mesas_infantil: result.rows[0] });
...
app.put('/api/mesas/:id', async (req, res) => {
  try {
    ...
    const result = await db.query(
      'UPDATE mesas_infantil SET nome = $1, cadeiras = $2, ocupada = true WHERE id = $3 RETURNING *',
      [nome, cadeiras, id]
    );
    ...
    res.status(200).json({ message: 'Mesa atualizada com sucesso.', mesas_infantil: result.rows[0] });
    broadcastMesasAtualizadas(); 
  } catch (error) {
...
app.put('/api/mesas/limpar/:id', async (req, res) => {
  try {
    ...
    const result = await db.query(
      'UPDATE mesas_infantil SET nome = $1, cadeiras = $2, ocupada = $3 WHERE id = $4 RETURNING *',
